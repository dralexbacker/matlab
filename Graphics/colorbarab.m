function [handle,him]=colorbarab(loc)
%COLORBARAB Display color bar (color scale).
% Same as COLORBAR but returns a handle to the image object.
%
...
if loc(1)=='v', % Append vertical scale to right of current plot
    ...
    % Create color stripe
    n = size(colormap,1);
    him=image([0 1],t,(1:n)','Tag','TMW_COLORBAR','deletefcn','colorbar(''delete'')');     
    set(ax,'Ydir','normal')
...
elseif loc(1)=='h', % Append horizontal scale to top of current plot *******************************
    ...
    % Create color stripe
    n = size(colormap,1);
    him=image(t,[0 1],(1:n),'Tag','TMW_COLORBAR','deletefcn','colorbar(''delete'')'); 
    set(ax,'Ydir','normal')
    set(ax,'ytick',[])
