function hpol = polarab(theta,rho,rmax,line_style)
%POLARab  Polar coordinate plot.
%   POLAR(THETA, RHO) makes a plot using polar coordinates of
%   the angle THETA, in radians, versus the radius RHO.
%   POLAR(THETA,RHO,RMAX) uses RMAX maximum radius
%   POLAR(THETA,RHO,RMAX,S) uses the linestyle specified in string S.
...
elseif nargin == 2 | (nargin == 3  & ~isempty(rho))
    if isstr(rho)
        line_style = rho;
        rho = theta;
        [mr,nr] = size(rho);
...
elseif nargin == 1 | (nargin == 3 & isempty(rho))
    line_style = 'auto';
    rho = theta;
    [mr,nr] = size(rho);
    if mr == 1
