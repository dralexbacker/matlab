function fout = timebar(x,whichbar, varargin)
%   TIMEBAR Display wait bar with time left estimator
%
%   Call first time with h=timebar(0), then with
%   timebar(fraction completed, h)
%
%   H = TIMEBAR(X,'title', property, value, property, value, ...) 
...
%
%   TIMEBAR(X) will set the length of the bar in the most recently
%   created waitbar window to the fractional length X.
%
%   TIMEBAR(X,H) will set the length of the bar in waitbar H
%   to the fractional length X.
%
%   TIMEBAR(X,H,'updated title') will update the title text in
%   the waitbar figure, in addition to setting the fractional
%   length to X.
%
%   TIMEBAR is typically used inside a FOR loop that performs a 
%   lengthy computation.  A sample usage is shown below:
%
%       h = timebar(0,'Please wait...');
%       for i=1:100,
%           % computation here %
%           timebar(i/100,h)
%       end
...
%   Modification Copyright Alex Backer Nov 2001
...
if nargin>=2
    ...
    elseif isnumeric(whichbar)
        ...
        if x,
            ...
            togo=elapsed*100; % arbitrary, cannot estimate at the very beginning
        end
        togomin=decplac(togo/60,2);
    else
...
switch type
 case 1,  % waitbar(x)    update
  ...
  if x,
      if nargin>2,
          tit=[varargin{1},' ',num2str(togomin),' min left'];
      else,
          tit=[num2str(togomin),' min left'];
      end
      set(hTitle,'string',tit);
  end
