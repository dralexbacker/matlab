function [m,celda]=dlmreadab(filename, dlm, r, c, rng)
%DLMREADAB Read ASCII delimited file. Numeric data returned in M, rest returned in CELDA.
% If a colum consists of dates (e.g. 2005-09-06), a strange number will be
% returned in M, and nothing in CELDA.
    ...
    while i <= length(line)
        ...
        if(all | ((loc >= ulc) & (loc <= brc)))
            if ~isempty(num)
                ...
                if ~isempty(v) % numbers give non-empty
                    m(loc(2)+r, loc(1)+c) = v;
                else % try a char
                    if length(setstr(num))==1, % if single character, can use matrix of string letters
                       m(loc(2)+r, loc(1)+c) = setstr(num);
                    else, % column's input is a multicharacter string, use cell
