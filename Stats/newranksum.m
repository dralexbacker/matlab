...
if any(s2==0),
    if s2==[0 0]',
        if x(1)~=y(1), % homogeneous populations different from each other are unlikely to be identical
            p=0;
        elseif x(1)==y(1), % homegeneous identical populations are likely to be identical
            p=1;
        end
    elseif max(x)<min(y) | max(y)<min(x), % non-overlapping populations, one population with zero variance, are not likely to be the same
        p=0;                                % this statement is weaker than the previous two, so it could be eliminated
    end
end
