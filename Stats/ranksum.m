...
% Modified by Alex Backer, March 2001, to handle zero variance in large samples
% Also called Mann-Whitney test (Freund & Wilson: Statistical Methods, 2nd Ed, 1997, Academic Press, p599).
...
%   See also SPEARMAN
%
%   Hogg and Tanis: Probability and Statistical Inference, 4th Ed: 2-sample Wilcoxon does not assume symmetry around median, 1-sample one does
%
%   Maximum resolution in p-value: 0.0001, limited by resolution of normcdf.m
...
if ns < 10 & (nx+ny) < 20     % Use the sampling distribution of W.
   ...
   if wvar==0, % Addition Alex Backer, March 2001
       wvar=10^(-7);
   end
	z = (w-wmean)/sqrt(wvar);
	p = normcdf(z,0,1);
